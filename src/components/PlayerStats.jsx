...
import { Skull, Heart, Swords } from 'lucide-react';
...
const GeneralStats = ({ stats }) => (
    <div className="bg-gray-800 p-4 rounded-lg">
        ...
        <StatRow icon={<Swords className="w-5 h-5 text-red-400" />} label="Eliminations" value={formatNumber(stats.total.eliminations)} />
                ...
                <StatRow icon={<Swords className="w-5 h-5 text-blue-400" />} label="KDA" value={stats.kda ?? 'N/A'} />
                <StatRow icon={<Swords className="w-5 h-5 text-yellow-400" />} label="Winrate" value={`${stats.winrate ?? 'N/A'}%`} />
                <StatRow icon={<Swords className="w-5 h-5 text-purple-400" />} label="Games Played" value={formatNumber(stats.games_played)} />
                <StatRow icon={<Swords className="w-5 h-5 text-pink-400" />} label="Games Won" value={formatNumber(stats.games_won)} />
                <StatRow icon={<Swords className="w-5 h-5 text-indigo-400" />} label="Time Played" value={formatPlayTime(stats.time_played)} />
        </div>
    </div>
);
...
const AverageStats = ({ stats }) => (
    <div className="bg-gray-800 p-4 rounded-lg">
        ...
        <div className="grid grid-cols-2 gap-4">
            <StatRow label="Eliminations" value={stats.average.eliminations.toFixed(2)} />
            <StatRow label="Assists" value={stats.average.assists.toFixed(2)} />
            <StatRow label="Deaths" value={stats.average.deaths.toFixed(2)} />
            <StatRow label="Damage" value={formatNumber(stats.average.damage)} />
            <StatRow label="Healing" value={formatNumber(stats.average.healing)} />
        </div>
    </div>
);
...
const RoleStats = ({ role, stats }) => (
    <div className="bg-gray-800 p-4 rounded-lg">
        ...
        <div className="grid grid-cols-2 gap-4">
            <StatRow label="Games Played" value={formatNumber(stats.games_played)} />
            <StatRow label="Games Won" value={formatNumber(stats.games_won)} />
            <StatRow label="Win Rate" value={`${stats.winrate}%`} />
            <StatRow label="KDA" value={stats.kda.toFixed(2)} />
            <StatRow label="Time Played" value={formatPlayTime(stats.time_played)} />
        </div>
        <h4 className="text-lg font-bold mt-4">Average</h4>
        <div className="grid grid-cols-2 gap-4">
            <StatRow label="Eliminations" value={stats.average.eliminations.toFixed(2)} />
            <StatRow label="Assists" value={stats.average.assists.toFixed(2)} />
            <StatRow label="Deaths" value={stats.average.deaths.toFixed(2)} />
